...
public:
	...
	std::string printItem(int) const;
		//Function to return the data contained at this specific node.
		//Postcondition: none
		//Added by Anthony Castillo on 7/20/2019
...
private: 
    void copyList(const linkedListType<Type>& otherList); // A helper function
...
template <class Type>
std::string linkedListType<Type>::printItem(int requested) const
{
	// Added by Anthony Castillo on 7/20/2019
	nodeType<Type> *current; //pointer to traverse the list
	...
	if (requested < count)
	{
		...
		{
			if (i == requested)// if we've reached the requested node
			{
				return current->info;
			}
			else if (current == nullptr)// We've reached the end of the list
			{
				cout << "Could not find that node." << endl;
				return "";
			}
			else if (current->link != nullptr)//if we haven't reached the last node
			{
				current = current->link;
			}
			else
